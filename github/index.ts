...
import { createLocalcodeClient } from "@localcode/sdk"
...
const { client, server } = createLocalcode()
...
try {
  ...
  await assertLocalcodeConnected()
  ...
  console.log("localcode session", session.id)
  if (shareId) {
...
function createLocalcode() {
  ...
  const proc = spawn(`localcode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createLocalcodeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/localcode|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/localcode` or `/oc`")
  }
}
...
async function assertLocalcodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to localcode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.localcode.dev" : "https://localcode.dev"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.localcode.dev/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("localcode-github-action")
    response = await fetch("https://api.localcode.dev/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/localcode" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/localcode") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to localcode...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "localcode-agent[bot]"`
  await $`git config --global user.email "localcode-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `localcode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/localcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[localcode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""

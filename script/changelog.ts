...
import { $ } from "bun"
import { createLocalcode } from "@localcode/sdk/v2"
import { parseArgs } from "util"
import { Script } from "@localcode/script"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/anomalyco/localcode/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/anomalyco/localcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/localcode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/localcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/localcode/")) areas.add("core")
...
async function summarizeCommit(localcode: Awaited<ReturnType<typeof createLocalcode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await localcode.client.session.create()
  const result = await localcode.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "localcode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], localcode: Awaited<ReturnType<typeof createLocalcode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(localcode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/anomalyco/localcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const localcode = await createLocalcode({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, localcode)
    notes.push(...lines)

...
- `localcode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `localcode web` view. **Do not** add for terminal, TUI, or general localcode issues.
...
**Only** add if the issue mentions "zen" or "localcode zen" or "localcode black".

If the issue doesn't have "zen" or "localcode black" in it then don't add zen label

#### core

Use for core server issues in `packages/localcode/`, excluding `packages/localcode/src/cli/cmd/tui/`.
...
TUI (`packages/localcode/src/cli/cmd/tui/...`):
...
Core (`packages/localcode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for localcode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)

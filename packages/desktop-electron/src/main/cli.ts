...
const CLI_INSTALL_DIR = ".localcode/bin"
const CLI_BINARY_NAME = "localcode"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `localcode-cli${suffix}`)
    : join(root, "../../resources", `localcode-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "localcode-install.sh")
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.localcode/bin/localcode"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://localcode.dev/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",

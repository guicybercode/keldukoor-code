...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.localcode.desktop.dev",
  beta: "ai.localcode.desktop.beta",
  prod: "ai.localcode.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.localcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `localcode.settings.dat` is special: it maps to the `localcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"localcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // localcode.settings.dat → the electron settings store ("localcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "localcode.global.dat").
  const storeName = filename === "localcode.settings.dat" ? "localcode.settings" : filename

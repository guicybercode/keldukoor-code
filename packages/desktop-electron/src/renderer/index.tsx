...
import "./styles.css"
import { useTheme } from "@localcode/ui/theme"
...
const deepLinkEvent = "localcode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__LOCALCODE__ ??= {}
  const pending = window.__LOCALCODE__.deepLinks ?? []
  window.__LOCALCODE__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__LOCALCODE__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__LOCALCODE__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__LOCALCODE__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://localcode.dev/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__LOCALCODE__!.wsl ?? false
    },
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("localcode.global.dat").getItem("language")

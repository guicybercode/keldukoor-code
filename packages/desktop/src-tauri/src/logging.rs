...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("localcode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("localcode_lib=debug,localcode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("localcode_lib=info,localcode_desktop=info,sidecar=info")
        }
    });

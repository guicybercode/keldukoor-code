...
Replace runtime Bun shell template-tag usage in `packages/localcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/localcode/src` is removed except:

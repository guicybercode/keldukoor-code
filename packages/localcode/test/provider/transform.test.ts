...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const localcodeModel = {
      ...openaiModel,
      providerID: "localcode",
      api: {
        id: "localcode-test",
        url: "https://api.localcode.dev",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, localcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.localcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.localcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          localcode: { itemId: "msg_localcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              localcode: { itemId: "msg_localcode_part" },
    ...
    expect(result[0].providerOptions?.localcode?.itemId).toBe("msg_localcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.localcode?.itemId).toBe("msg_localcode_part")

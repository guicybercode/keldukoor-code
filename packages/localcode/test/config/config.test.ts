...
const managedConfigDir = process.env.LOCALCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "localcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "localcode.json") {
...
test("ignores legacy tui keys in localcode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://localcode.dev/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "localcode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["LOCALCODE_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      localcode: {
        options: {
          apiKey: "{env:LOCALCODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["localcode"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["LOCALCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["LOCALCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "localcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .localcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcodeDir = path.join(dir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })
      const agentDir = path.join(localcodeDir, "agent")
...
test("loads agents from .localcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcodeDir = path.join(dir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })

      const agentsDir = path.join(localcodeDir, "agents")
...
test("loads commands from .localcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcodeDir = path.join(dir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })

      const commandDir = path.join(localcodeDir, "command")
...
test("loads commands from .localcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const localcodeDir = path.join(dir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })

      const commandsDir = path.join(localcodeDir, "commands")
...
test("does not try to install dependencies in read-only LOCALCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.LOCALCODE_CONFIG_DIR
  process.env.LOCALCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LOCALCODE_CONFIG_DIR
    else process.env.LOCALCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable LOCALCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "localcode.json"),
        JSON.stringify({ $schema: "https://localcode.dev/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "localcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .localcode config
      const projectDir = path.join(dir, "project")
      const localcodeDir = path.join(projectDir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })
      ...
      // Local .localcode config with different plugins
      await Filesystem.write(
        path.join(localcodeDir, "localcode.json"),
        JSON.stringify({
          $schema: "https://localcode.dev/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .localcode config with some overlapping plugins
      await Filesystem.write(
        path.join(localcodeDir, "localcode.json"),
        JSON.stringify({
          $schema: "https://localcode.dev/config.json",
...
// Note: preload.ts sets LOCALCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .localcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .localcode directory config enables it
      const localcodeDir = path.join(dir, ".localcode")
      await fs.mkdir(localcodeDir, { recursive: true })
      await Filesystem.write(
        path.join(localcodeDir, "localcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/localcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/localcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-localcode@2.4.3")).toBe("oh-my-localcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@localcode/plugin@2.0.0")).toBe("@localcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-localcode@2.4.3", "file:///project/.localcode/plugin/oh-my-localcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.localcode/plugin/oh-my-localcode.js")
  })
  ...
  test("local plugin directory overrides global localcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const localcodeDir = path.join(projectDir, ".localcode")
        const pluginDir = path.join(localcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LOCALCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"]
    process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .localcode/ directories when flag is set", async () => {
    const originalEnv = process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"]
    process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .localcode directory with a command
          const localcodeDir = path.join(dir, ".localcode", "command")
          await fs.mkdir(localcodeDir, { recursive: true })
          await Filesystem.write(path.join(localcodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .localcode should NOT be in directories list
          const hasProjectLocalcode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectLocalcode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOCALCODE_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LOCALCODE_CONFIG_DIR"]
      } else {
        process.env["LOCALCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LOCALCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LOCALCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LOCALCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LOCALCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from LOCALCODE_CONFIG_DIR, not project
...
describe("LOCALCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in LOCALCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LOCALCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["LOCALCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://localcode.dev/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["LOCALCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["LOCALCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in LOCALCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["LOCALCODE_CONFIG_CONTENT"]

...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.LOCALCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.LOCALCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.LOCALCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".localcode") && dir !== Flag.LOCALCODE_CONFIG_DIR) continue

...
const TUI_SCHEMA_URL = "https://localcode.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from localcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const localcode = await localcodeFiles(input)
...
async function localcodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.LOCALCODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("localcode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "localcode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "localcode"))
  }
  if (Flag.LOCALCODE_CONFIG) files.push(Flag.LOCALCODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "localcode"))

...
export namespace Database {
  export function getChannelPath() {
    ...
    if (["latest", "beta"].includes(channel) || Flag.LOCALCODE_DISABLE_CHANNEL_DB)
      return path.join(Global.Path.data, "localcode.db")
    const safe = channel.replace(/[^a-zA-Z0-9._-]/g, "-")
    return path.join(Global.Path.data, `localcode-${safe}.db`)
  }

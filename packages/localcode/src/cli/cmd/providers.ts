...
import type { Hooks } from "@localcode/plugin"
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "localcode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          ...
          const wellknown = await fetch(`${url}/.well-known/localcode`).then((x) => x.json() as any)
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in localcode.json, check the docs for examples.`,
          )
        }

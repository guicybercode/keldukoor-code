...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run localcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for localcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found localcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["localcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting localcode...")
        UI.println()

        const localcodeArgs = sessionId ? ["-s", sessionId] : []
        const localcodeProcess = Process.spawn(["localcode", ...localcodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await localcodeProcess.exited
        if (code !== 0) throw new Error(`localcode exited with code ${code}`)
      },
    })
  },
})

...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .localcode/ subdirectory too)
  const candidates = [path.join(baseDir, "localcode.json"), path.join(baseDir, "localcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".localcode", "localcode.json"), path.join(baseDir, ".localcode", "localcode.jsonc"))
  }
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "localcode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })

...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g localcode",
      pnpm: "pnpm uninstall -g localcode",
      bun: "bun remove -g localcode",
      yarn: "yarn global remove localcode",
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# localcode") || content.includes(".localcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".localcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".localcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".localcode"))
    ) {
      continue
    }

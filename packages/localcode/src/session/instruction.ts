...
function globalFiles() {
  ...
  if (Flag.LOCALCODE_CONFIG_DIR) {
    files.push(path.join(Flag.LOCALCODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.LOCALCODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.LOCALCODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no LOCALCODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.LOCALCODE_CONFIG_DIR, Flag.LOCALCODE_CONFIG_DIR).catch(() => [])
}

...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for localcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal localcode sessions
...
localcode acp --cwd /path/to/project
```
...
```bash
LOCALCODE_ENABLE_QUESTION_TOOL=1 localcode acp
```
...
- **Terminal Integration**: Full terminal support via localcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | localcode acp
```
...
### Mapping to LocalCode

ACP sessions map cleanly to localcode's internal session model:

...
import type { AssistantMessage, Event, LocalcodeClient, SessionMessageResponse, ToolPart } from "@localcode/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: LocalcodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `localcode auth login` in the terminal",
        name: "Login with localcode",
        id: "localcode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // LocalCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    return { providerID: ProviderID.ollama, modelID: ModelID.make(models[0]?.id ?? "") }
  }

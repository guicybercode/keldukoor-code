...
export function buildCostChunk(_format: ZenData.Format, cost: string): string {
  return `data: ${JSON.stringify({ choices: [], cost })}\n\n`
}

export function createBodyConverter(_from: ZenData.Format, _to: ZenData.Format) {
  return (body: any): any => {
    const raw: CommonRequest = fromOaCompatibleRequest(body)
    return toOaCompatibleRequest(raw)
  }
}

export function createStreamPartConverter(_from: ZenData.Format, _to: ZenData.Format) {
  return (part: any): any => {
    const raw: CommonChunk | string = fromOaCompatibleChunk(part)
    if (typeof raw === "string") return raw
    return toOaCompatibleChunk(raw)
  }
}

export function createResponseConverter(_from: ZenData.Format, _to: ZenData.Format) {
  return (response: any): any => {
    const raw: CommonResponse = fromOaCompatibleResponse(response)
    return toOaCompatibleResponse(raw)
  }
}

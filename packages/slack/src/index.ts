...
import { createLocalcode, type ToolPart } from "@localcode/sdk"
...
console.log("🚀 Starting localcode server...")
const localcode = await createLocalcode({
  port: 0,
})
console.log("✅ Localcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await localcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new localcode session...")
    const { client, server } = localcode
    ...
    console.log("✅ Created localcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to localcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Localcode response:", JSON.stringify(result, null, 2))

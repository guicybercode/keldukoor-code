---
title: Server
description: Komunicirajte s localcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `localcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji localcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `localcode.local` |
...
Postavite `LOCALCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `localcode`, ili postavite `LOCALCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `localcode serve` i za `localcode web`.
...
Kada pokrenete `localcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite localcode server za programsku interakciju sa localcode-om.
:::

Ova arhitektura omogucava localcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `localcode serve` da startate standalone server. Ako je localcode TUI vec pokrenut, `localcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste LocalCode [IDE](/docs/ide) pluginovi.
...
localcode server izlaže sljedece API-je.

...
ove ili konfigurirajte svoje putem localcode config.
...
localcode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.localcode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `localcode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/localcode/modes/`
- Projekat: `.localcode/modes/`
...
Ako temperatura nije navedena, localcode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.localcode/modes/` za specifične načine rada ili `~/.config/localcode/modes/` za globalne načine:

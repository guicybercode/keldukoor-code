...
Postoje dvije vrste agenata u LocalCode: primarni agenti i podagenti.
...
LocalCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
LocalCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

LocalCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `localcode.json`:
...
- Globalno: `~/.config/localcode/agents/`
- Po projektu: `.localcode/agents/`
...
Ako temperatura nije navedena, LocalCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu LocalCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj LocalCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [LocalCode Zen](/docs/zen), koristili biste `localcode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `localcode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/localcode).
:::

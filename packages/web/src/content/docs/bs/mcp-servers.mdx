...
Možete dodati vanjske alate u LocalCode koristeći _Model Context Protocol_, ili MCP. LocalCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [LocalCode Config](https://localcode.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/localcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
LocalCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, LocalCode će:
...
Ako server zahtijeva autentifikaciju, LocalCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `localcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, LocalCode sigurno čuva tokene u `~/.local/share/localcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u LocalCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz LocalCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete LocalCode sa vašim Sentry nalogom.

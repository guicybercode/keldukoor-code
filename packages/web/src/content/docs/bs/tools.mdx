...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. LocalCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u LocalCode.
...
:::note
Ovaj alat je dostupan samo kada je `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `LOCALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz LocalCode provajdera ili kada je varijabla `LOCALCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju LocalCode:

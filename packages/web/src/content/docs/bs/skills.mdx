...
Agent skills omogucavaju LocalCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
LocalCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.localcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/localcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, LocalCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.localcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.localcode/skills/git-release/SKILL.md` ovako:
...
LocalCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `localcode.json`:
...
**Za ugradene agente** (u `localcode.json`):

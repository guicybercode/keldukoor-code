...
Da biste otklonili probleme s LocalCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\localcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `localcode --log-level DEBUG`.

---

## Pohrana

LocalCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/localcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\localcode`
...
LocalCode Desktop pokreće lokalni LocalCode server (`localcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `LocalCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/localcode/localcode.jsonc` (ili `~/.config/localcode/localcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/localcode/localcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\localcode\localcode.jsonc`
...
LocalCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/localcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\localcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga LocalCode mogao ponovo izgraditi.

1. Potpuno zatvorite LocalCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/localcode`
- **Linux**: obrišite `~/.cache/localcode` (ili pokrenite `rm -rf ~/.cache/localcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\localcode`

3. Ponovo pokrenite LocalCode Desktop.

---

### Rješavanje problema sa vezom na serveru

LocalCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `localcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LOCALCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LOCALCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, LocalCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa LocalCode karakteristikama.

---

### Obavještenja se ne prikazuju

LocalCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za LocalCode u postavkama vašeg OS-a, i
...
1. Zatvorite LocalCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka LocalCode Desktop aplikacije):

- `localcode.settings.dat` (URL zadanog servera za desktop)
- `localcode.global.dat` i `localcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s LocalCode:
...
Da saznate kojim modelima imate pristup, pokrenite `localcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\localcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. LocalCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\localcode`

2. Ponovo pokrenite LocalCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti LocalCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
LocalCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

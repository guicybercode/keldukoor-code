...
U LocalCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, LocalCode koristi nasu `localcode` temu.
...
LocalCode dolazi sa vise ugradenih tema.
...
- Zele da LocalCode odgovara izgledu njihovog terminala
...
LocalCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/localcode/themes/*.json` ili `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.localcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.localcode/themes/*.json`

---
title: Konfiguracija
description: Korištenje LocalCode JSON konfiguracije.
---

Možete konfigurirati LocalCode koristeći JSON konfiguracijski fajl.

---

## Format

LocalCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/localcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/localcode/localcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`LOCALCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`localcode.json` u projektu) - postavke specifične za projekat
5. **`.localcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`LOCALCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.localcode` i `~/.config/localcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/localcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu LocalCode konfiguraciju u `~/.config/localcode/localcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/localcode/tui.json`.
...
Dodajte `localcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se LocalCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `LOCALCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `LOCALCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.localcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.localcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`localcode.dev/config.json`**](https://localcode.dev/config.json).

TUI konfiguracija koristi [**`localcode.dev/tui.json`**](https://localcode.dev/tui.json).
...
Koristite `LOCALCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `localcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `localcode serve` i `localcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš LocalCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `localcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj LocalCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, LocalCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/localcode/agents/` ili `.localcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, LocalCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`localcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/localcode/commands/` ili `.localcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
LocalCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, LocalCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju LocalCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.localcode/plugins/` ili `~/.config/localcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite LocalCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

---
title: Dodaci
description: Napišite vlastite dodatke za proširenje LocalCode.
---

Dodaci vam omogućavaju da proširite LocalCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja LocalCode.
...
- `.localcode/plugins/` - Dodaci na nivou projekta
- `~/.config/localcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/localcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/localcode/localcode.json`)
2. Konfiguracija projekta (`localcode.json`)
3. Globalni direktorij dodataka (`~/.config/localcode/plugins/`)
4. Direktorij dodataka projekta (`.localcode/plugins/`)
...
LocalCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Localcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje LocalCode.
...
Spriječite localcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u localcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji localcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za localcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://localcode.dev/docs/sdk) za detalje.

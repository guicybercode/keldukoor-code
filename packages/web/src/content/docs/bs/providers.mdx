---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj LocalCode konfiguraciji.
...
u `~/.local/share/localcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem LocalCode
config.
...
LocalCode Zen je lista modela koje je obezbedio LocalCode tim koji su bili
testirano i potvrđeno da dobro radi sa LocalCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa LocalCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite localcode i idite na [localcode.dev/auth](https://localcode.dev/auth).
...
Radi kao i svaki drugi provajder u LocalCode i potpuno je opcionalan za korištenje.

---

## LocalCode Go

LocalCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža LocalCode tim i koji su testirani i verificirani da dobro rade s LocalCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `LocalCode Go` i idite na [localcode.dev/auth](https://localcode.dev/zen).
...
Da biste koristili Amazon Bedrock s LocalCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete localcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `localcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u LocalCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi localcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju localcode konfiguraciju.
...
da pohrani token u localcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
LocalCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate LocalCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`localcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `localcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s localcode:
...
Za korištenje Google Vertex AI s LocalCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće localcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem localcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) da automatski prijavite svaki LocalCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki LocalCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati localcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati localcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati localcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za LocalCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/localcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s LocalCode:
...
3. Kopirajte API ključ za korištenje u LocalCode.
...
6. **Važno**: Prije upotrebe modela oblaka u LocalCode, morate lokalno povući informacije o modelu:
...
LocalCode Zen je lista testiranih i verifikovanih modela koju je obezbedio LocalCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>LocalCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **LocalCode Zen**.

```txt
   /connect
```

3. Unesite svoj LocalCode API ključ.
...
Također možete dodati dodatne modele putem vaše localcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše localcode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Localcodeom:
...
Također možete prilagoditi modele kroz svoju localcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s localcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `localcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju LocalCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `localcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite LocalCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj localcode konfiguraciji.

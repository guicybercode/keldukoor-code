...
With LocalCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, LocalCode uses our own `localcode` theme.
...
LocalCode comes with several built-in themes.
...
- Want LocalCode to match their terminal's appearance
...
LocalCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/localcode/themes/*.json` or `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.localcode/themes/*.json`
4. **Current working directory** - Defined in `./.localcode/themes/*.json`

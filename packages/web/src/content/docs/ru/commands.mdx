...
Создайте `.localcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию localcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем localcode [config](/docs/config):
...
- Глобальный: `~/.config/localcode/commands/`
- Для каждого проекта: `.localcode/commands/`
...
localcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).

...
Med LocalCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger LocalCode vårt eget `localcode`-tema.
...
LocalCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/localcode/themes/*.json` eller `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.localcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.localcode/themes/*.json`

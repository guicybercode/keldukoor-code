...
For at fejlfinde problemer med LocalCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\localcode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `localcode --log-level DEBUG`.

---

## Lagring

localcode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/localcode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\localcode`
...
LocalCode Desktop kører en lokal LocalCode-server (`localcode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `LocalCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/localcode/localcode.jsonc` (eller `~/.config/localcode/localcode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/localcode/localcode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\localcode\localcode.jsonc`
...
LocalCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/localcode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\localcode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så LocalCode kan genopbygge den.

1. Afslut LocalCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/localcode`
- **Linux**: slet `~/.cache/localcode` (eller kør `rm -rf ~/.cache/localcode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\localcode`

3. Start LocalCode Desktop på ny.

---

### Løs problemer med serverforbindelse

LocalCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `localcode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LOCALCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LOCALCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver LocalCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i LocalCode.

---

### Meddelelser vises ikke

LocalCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for LocalCode i dine OS-indstillinger, og
...
1. Afslut LocalCode Desktop.
2. Find og slet disse filer (de findes i LocalCode Desktop-appens datamappe):

- `localcode.settings.dat` (skrivebordsstandardserver URL)
- `localcode.global.dat` og `localcode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med LocalCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `localcode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\localcode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. localcode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\localcode`

2. Start localcode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge localcode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
localcode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.

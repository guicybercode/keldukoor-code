...
Agentferdigheter lar LocalCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
LocalCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.localcode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/localcode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går LocalCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.localcode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.localcode/skills/git-release/SKILL.md` slik:
...
LocalCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `localcode.json`:
...
**For innebygde agenter** (i `localcode.json`):

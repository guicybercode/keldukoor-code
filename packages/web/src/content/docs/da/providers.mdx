---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i LocalCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/localcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i LocalCode-konfigurationen.
...
LocalCode Zen er en liste over modeller leveret af LocalCode-teamet, som er blevet testet og verificeret til at fungere godt med LocalCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med LocalCode Zen.
:::
...
Det fungerer som alle andre udbydere i LocalCode og er helt valgfrit at bruge.

---

## LocalCode Go

LocalCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af LocalCode-teamet, som er testet og verificeret til at fungere godt med LocalCode.

1. Kør kommandoen `/connect` i TUI, vælg `LocalCode Go`, og gå til [localcode.dev/auth](https://localcode.dev/zen).
...
Sådan bruger du Amazon Bedrock med LocalCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører localcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `localcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i LocalCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at localcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem localcode-konfigurationen.
...
gemme tokenet i localcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
LocalCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse LocalCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`localcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `localcode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med LocalCode:
...
Sådan bruger du Google Vertex AI med LocalCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører localcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem localcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) plugin for automatisk at logge hver LocalCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver LocalCode-samtale opført som en separat session.
...
Du kan konfigurere localcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere localcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere localcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for LocalCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/localcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med LocalCode:
...
3. Kopier API-nøglen til brug i LocalCode.
...
6. **Vigtigt**: Før du bruger skymodeller i LocalCode, skal du hente modelinformationen lokalt:
...
LocalCode Zen er en liste over testede og verificerede modeller leveret af LocalCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>LocalCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Indtast LocalCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem localcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem localcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LocalCode:
...
Du kan også tilpasse modeller gennem localcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med localcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `localcode.json`-filen i projektkataloget:
...
`limit`-felterne lader LocalCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `localcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek localcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i localcode-konfigurationen.

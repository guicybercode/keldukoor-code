...
Her er alle de innebygde verktøyene som er tilgængelige i LocalCode.
...
:::note
Dette verktøyet er bare tilgængelig når `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `LOCALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger LocalCode-leverandøren eller når miljøvariabelen `LOCALCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

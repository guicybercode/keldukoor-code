---
title: Server
description: Interager med localcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `localcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en localcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `localcode.local` |
...
Indstil `LOCALCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `localcode`, eller indstil `LOCALCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `localcode serve` og `localcode web`.
...
Når du kører `localcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug localcode-serveren til at interagere med localcode programmatisk.
:::

Denne arkitektur lader localcode understøtte flere klienter og lader dig interagere med localcode programmatisk.

Du kan køre `localcode serve` for at starte en selvstændig server. Hvis du har
localcode TUI kørende, vil `localcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af LocalCode [IDE](/docs/ide) plugins.
...
LocalCode-serveren viser følgende API'er.

...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i localcode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i localcode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem localcode-konfigurationen.
...
localcode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.localcode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `localcode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger localcode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale localcode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.localcode/modes/` for projektspecifikke tilstande eller `~/.config/localcode/modes/` for globale tilstande:

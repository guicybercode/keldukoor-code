---
title: Plugins
description: Skriv dine egne plugins for at udvide LocalCode.
---

Plugins giver dig mulighed for at udvide LocalCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre LocalCodes standardadfærd.
...
- `.localcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/localcode/node_modules/`.
...
1. Global konfiguration (`~/.config/localcode/localcode.json`)
2. Projektkonfiguration (`localcode.json`)
3. Global plugin-mappe (`~/.config/localcode/plugins/`)
4. Projekt plugin bibliotek (`.localcode/plugins/`)
...
LocalCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En localcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide localcode.
...
:::note
Hvis du bruger LocalCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå localcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til localcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som localcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for localcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://localcode.dev/docs/sdk) for detaljer.

---
title: Server
description: Interagisci con il server localcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `localcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client localcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `localcode.local` |
...
Imposta `LOCALCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `localcode`, oppure imposta `LOCALCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `localcode serve` sia per `localcode web`.
...
Quando esegui `localcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server localcode per interagire con localcode in modo programmatico.
:::

Questa architettura permette a localcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `localcode serve` per avviare un server standalone. Se la TUI di localcode e gia in esecuzione, `localcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di LocalCode.
...
Il server localcode espone le seguenti API.

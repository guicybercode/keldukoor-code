...
Con LocalCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, LocalCode usa il tema `localcode`.
...
LocalCode include diversi temi integrati.
...
- Vuole che LocalCode corrisponda all'aspetto del terminale
...
LocalCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/localcode/themes/*.json` o `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.localcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.localcode/themes/*.json`

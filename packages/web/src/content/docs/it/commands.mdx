...
Crea `.localcode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di LocalCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di LocalCode:
...
- Globale: `~/.config/localcode/commands/`
- Per progetto: `.localcode/commands/`
...
localcode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).

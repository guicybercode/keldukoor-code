...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di localcode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in localcode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di localcode.
...
localcode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.localcode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `localcode.json`:
...
- Globale: `~/.config/localcode/modes/`
- Progetto: `.localcode/modes/`
...
Se non viene specificata alcuna temperatura, localcode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di localcode sia per quella specifica del progetto.
...
Crea file di modalita in `.localcode/modes/` per modalita specifiche del progetto oppure in `~/.config/localcode/modes/` per modalita globali:

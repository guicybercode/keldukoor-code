---
title: Plugin
description: Scrivi plugin per estendere LocalCode.
---

I plugin ti permettono di estendere LocalCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di LocalCode.
...
- `.localcode/plugins/` - plugin a livello progetto
- `~/.config/localcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/localcode/node_modules/`.
...
1. Config globale (`~/.config/localcode/localcode.json`)
2. Config di progetto (`localcode.json`)
3. Directory plugin globale (`~/.config/localcode/plugins/`)
4. Directory plugin di progetto (`.localcode/plugins/`)
...
LocalCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di localcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere localcode.
...
:::note
Se usi l'app desktop di LocalCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a localcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a localcode:
...
L'helper `tool` crea uno strumento personalizzato che localcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in localcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://localcode.dev/docs/sdk) per i dettagli.

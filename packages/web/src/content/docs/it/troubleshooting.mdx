...
Per diagnosticare problemi con LocalCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\localcode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `localcode --log-level DEBUG`.

---

## Archiviazione

localcode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/localcode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\localcode`
...
LocalCode Desktop esegue in background un server locale di LocalCode (il sidecar `localcode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `LocalCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/localcode/localcode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\localcode\localcode.jsonc`
...
LocalCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/localcode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\localcode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che LocalCode possa ricostruirla.

1. Chiudi completamente LocalCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/localcode`
- **Linux**: elimina `~/.cache/localcode` (oppure esegui `rm -rf ~/.cache/localcode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\localcode`

3. Riavvia LocalCode Desktop.

---

### Risolvi problemi di connessione al server

LocalCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `localcode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `LOCALCODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `LOCALCODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, LocalCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di LocalCode.

---

### Notifiche non visualizzate

LocalCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per LocalCode nelle impostazioni del sistema operativo, e
...
1. Chiudi LocalCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app LocalCode Desktop):

- `localcode.settings.dat` (desktop default server URL)
- `localcode.global.dat` e `localcode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con LocalCode:
...
Per capire a quali modelli hai accesso, esegui `localcode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\localcode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. localcode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\localcode`

2. Riavvia localcode per reinstallare i pacchetti provider piu recenti

Questo forzera localcode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
localcode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.

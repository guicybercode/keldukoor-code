...
In LocalCode esistono due tipi di agenti: agenti primari e subagenti.
...
LocalCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
LocalCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

LocalCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `localcode.json`:
...
- Globale: `~/.config/localcode/agents/`
- Per progetto: `.localcode/agents/`
...
Se non specifichi una temperature, LocalCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di LocalCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione LocalCode usa il formato `provider/model-id`. Per esempio, se stai usando [LocalCode Zen](/docs/zen), useresti `localcode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `localcode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/localcode).
:::

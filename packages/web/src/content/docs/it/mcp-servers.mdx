...
Puoi aggiungere strumenti esterni a LocalCode usando il _Model Context Protocol_, o MCP. LocalCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [LocalCode Config](https://localcode.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/localcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
LocalCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, LocalCode:
...
Se il server richiede autenticazione, LocalCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `localcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, LocalCode memorizzera' i token in modo sicuro in `~/.local/share/localcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in LocalCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione LocalCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare LocalCode al tuo account Sentry.

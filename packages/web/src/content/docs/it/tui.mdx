---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di LocalCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire LocalCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di LocalCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a LocalCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da LocalCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `localcode.json`, che configura il comportamento di server/runtime.
...
Usa `LOCALCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.

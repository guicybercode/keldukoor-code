...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. LocalCode include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in LocalCode.
...
:::note
Questo strumento e' disponibile solo quando `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (oppure `LOCALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider LocalCode o quando la variabile d'ambiente `LOCALCODE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di LocalCode:

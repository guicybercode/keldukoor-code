...
Le skill degli agenti permettono a LocalCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
LocalCode cerca in queste posizioni:

- Config di progetto: `.localcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/localcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, LocalCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.localcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.localcode/skills/git-release/SKILL.md` cosi':
...
LocalCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `localcode.json`:
...
**Per agenti integrati** (in `localcode.json`):

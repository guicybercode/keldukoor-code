---
title: Complementos
description: Escriba sus propios complementos para extender LocalCode.
---

Los complementos le permiten extender LocalCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de LocalCode.
...
- `.localcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/localcode/plugins/` - Complementos globales
...
```json title="localcode.json"
{
  ...
  "plugin": ["localcode-helicone-session", "localcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/localcode/node_modules/`.
...
1. Configuración global (`~/.config/localcode/localcode.json`)
2. Configuración del proyecto (`localcode.json`)
3. Directorio global de complementos (`~/.config/localcode/plugins/`)
4. Directorio de complementos del proyecto (`.localcode/plugins/`)
...
```json title=".localcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
LocalCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".localcode/plugins/my-plugin.ts"
...
```js title=".localcode/plugins/example.js"
...
- `client`: Un cliente SDK localcode para interactuar con la IA.
...
import type { Plugin } from "@localcode/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar localcode.
...
```js title=".localcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "localcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio LocalCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que localcode lea archivos `.env`:

```javascript title=".localcode/plugins/env-protection.js"
...
```javascript title=".localcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a localcode:

```ts title=".localcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@localcode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que localcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para localcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://localcode.dev/docs/sdk) para obtener más detalles.
...
```ts title=".localcode/plugins/compaction.ts"
import type { Plugin } from "@localcode/plugin"
...
```ts title=".localcode/plugins/custom-compaction.ts"
import type { Plugin } from "@localcode/plugin"

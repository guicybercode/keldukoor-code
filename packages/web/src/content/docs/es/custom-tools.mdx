...
description: Cree herramientas que LLM pueda llamar en localcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de localcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.localcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/localcode/tools/`.
...
```ts title=".localcode/tools/database.ts" {1}
...
```ts title=".localcode/tools/math.ts"
import { tool } from "@localcode/plugin"
...
```ts title=".localcode/tools/bash.ts"
import { tool } from "@localcode/plugin"
...
```ts title=".localcode/tools/project.ts" {8}
import { tool } from "@localcode/plugin"
...
```python title=".localcode/tools/add.py"
import sys
...
```ts title=".localcode/tools/python-add.ts" {10}
import { tool } from "@localcode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".localcode/tools/add.py")

...
Con LocalCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, LocalCode usa nuestro propio tema `localcode`.
...
LocalCode viene con varios temas integrados.
...
- Quiere que LocalCode coincida con la apariencia de su terminal
...
LocalCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/localcode/themes/*.json` o `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.localcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.localcode/themes/*.json`

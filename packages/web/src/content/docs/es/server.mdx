---
title: Servidor
description: Interactuar con el servidor localcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `localcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente localcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `localcode.local` |
...
Configure `LOCALCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `localcode`, o configure `LOCALCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `localcode serve` como a `localcode web`.
...
Cuando ejecuta `localcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor localcode para interactuar con localcode mediante programación.
:::

Esta arquitectura permite que localcode admita múltiples clientes y le permite interactuar con localcode mediante programación.

Puede ejecutar `localcode serve` para iniciar un servidor independiente. Si tienes el
localcode TUI ejecutándose, `localcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos LocalCode [IDE](/docs/ide).
...
El servidor localcode expone las siguientes API.

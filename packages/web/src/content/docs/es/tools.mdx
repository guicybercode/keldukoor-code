...
Las herramientas permiten que LLM realice acciones en su código base. LocalCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en LocalCode.
...
:::note
Esta herramienta solo está disponible cuando `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (o `LOCALCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor LocalCode o cuando la variable de entorno `LOCALCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar LocalCode:

```bash
LOCALCODE_ENABLE_EXA=1 localcode
```

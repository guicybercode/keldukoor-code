...
Puede agregar herramientas externas a LocalCode utilizando el _Model Context Protocol_ o MCP. LocalCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [LocalCode Config](https://localcode.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/localcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
LocalCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, LocalCode:
...
Si el servidor requiere autenticación, LocalCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `localcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, LocalCode almacenará los tokens de forma segura en `~/.local/share/localcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en LocalCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración LocalCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar LocalCode a su cuenta Sentry.

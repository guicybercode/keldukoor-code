...
Las habilidades del agente permiten a LocalCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
LocalCode busca estas ubicaciones:

- Configuración del proyecto: `.localcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/localcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, LocalCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.localcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.localcode/skills/git-release/SKILL.md` así:
...
LocalCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `localcode.json`:
...
**Para agentes integrados** (en `localcode.json`):

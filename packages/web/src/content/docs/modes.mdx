...
:::caution
Modes are now configured through the `agent` option in the localcode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in localcode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the localcode config.
...
localcode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.localcode/plans/*.md` to detail the plan itself
...
Configure modes in your `localcode.json` config file:
...
both the global localcode config and the project specific config.
...
Create mode files in `.localcode/modes/` for project-specific modes or `~/.config/localcode/modes/` for global modes:

...
Com o localcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o localcode usa nosso próprio tema `localcode`.
...
O localcode vem com vários temas embutidos.
...
- Querem que o localcode corresponda à aparência do seu terminal
...
O localcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/localcode/themes/*.json` ou `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.localcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.localcode/themes/*.json`

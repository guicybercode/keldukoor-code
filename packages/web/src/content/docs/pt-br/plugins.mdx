---
title: Plugins
description: Escreva seus próprios plugins para estender o localcode.
---

Plugins permitem que você estenda o localcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do localcode.
...
- `.localcode/plugins/` - Plugins em nível de projeto
- `~/.config/localcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/localcode/node_modules/`.
...
1. Configuração global (`~/.config/localcode/localcode.json`)
2. Configuração do projeto (`localcode.json`)
3. Diretório de plugins global (`~/.config/localcode/plugins/`)
4. Diretório de plugins do projeto (`.localcode/plugins/`)
...
O localcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do localcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o localcode.
...
:::note
Se você estiver usando o aplicativo desktop localcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o localcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao localcode:
...
O helper `tool` cria uma ferramenta personalizada que o localcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o localcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://localcode.dev/docs/sdk) para detalhes.

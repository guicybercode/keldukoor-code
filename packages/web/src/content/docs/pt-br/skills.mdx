...
As habilidades do agente permitem que o localcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O localcode pesquisa nesses locais:

- Configuração do projeto: `.localcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/localcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o localcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.localcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.localcode/skills/git-release/SKILL.md` assim:
...
O localcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `localcode.json`:
...
**Para agentes embutidos** (em `localcode.json`):

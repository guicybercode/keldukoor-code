---
title: Servidor
description: Interaja com o servidor localcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `localcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente localcode pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `localcode.local` |
...
Defina `LOCALCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `localcode`, ou defina `LOCALCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `localcode serve` quanto ao `localcode web`.

```bash
LOCALCODE_SERVER_PASSWORD=your-password localcode serve
```
...
Quando você executa `localcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor localcode para interagir com o localcode programaticamente.
:::

Essa arquitetura permite que o localcode suporte múltiplos clientes e permite que você interaja com o localcode programaticamente.

Você pode executar `localcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do localcode em execução, `localcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do localcode [IDE](/docs/ide).
...
O servidor localcode expõe as seguintes APIs.

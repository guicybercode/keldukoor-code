...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات localcode. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في localcode تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات localcode.
...
يأتي localcode مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.localcode/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `localcode.json` لديك:
...
- عالمي: `~/.config/localcode/modes/`
- للمشروع: `.localcode/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم localcode القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات localcode العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.localcode/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/localcode/modes/` لأوضاع عالمية:

...
يوجد نوعان من الوكلاء في LocalCode: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي LocalCode مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي LocalCode مع وكيلين فرعيين مدمجين: **General** و **Explore**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي LocalCode مع وكيلين أساسيين مدمجين ووكيلين فرعيين مدمجين.
...
هيّئ الوكلاء في ملف إعدادات `localcode.json`:
...
- عام: `~/.config/localcode/agents/`
- لكل مشروع: `.localcode/agents/`
...
إذا لم يتم تحديد temperature، يستخدم LocalCode القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات LocalCode العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات LocalCode الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [LocalCode Zen](/docs/zen)، فستستخدم `localcode/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `localcode models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/anomalyco/localcode).
:::

...
تتيح مهارات الوكيل لـ LocalCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث LocalCode في هذه المواقع:

- إعدادات المشروع: `.localcode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/localcode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج LocalCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.localcode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/localcode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.localcode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض LocalCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `localcode.json`:
...
**للوكلاء المدمجين** (في `localcode.json`):

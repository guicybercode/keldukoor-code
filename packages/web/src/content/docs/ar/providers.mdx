---
title: المزوّدون
description: استخدام أي موفّر LLM في LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم LocalCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات LocalCode.
...
في `~/.local/share/localcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات LocalCode.
...
LocalCode Zen هي قائمة نماذج يوفّرها فريق LocalCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع LocalCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع LocalCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر localcode، ثم انتقل إلى [localcode.dev/auth](https://localcode.dev/auth).
...
يعمل مثل أي مزوّد آخر في LocalCode واستخدامه اختياري بالكامل.

---

## LocalCode Go

LocalCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق LocalCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع LocalCode.

1. شغّل الأمر `/connect` في TUI، واختر `LocalCode Go`، ثم انتقل إلى [localcode.dev/auth](https://localcode.dev/zen).
...
لاستخدام Amazon Bedrock مع LocalCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل localcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `localcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل LocalCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل localcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات localcode.
...
تخزين الرمز في مساحة تخزين مصادقة localcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم LocalCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر LocalCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`localcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth).

##### التهيئة

خصّص عبر `localcode.json`:
...
لاستخدام اشتراك GitHub Copilot مع localcode:
...
لاستخدام Google Vertex AI مع LocalCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر localcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) لتسجيل كل محادثة LocalCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة LocalCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط localcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط localcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط localcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع LocalCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/localcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع LocalCode:
...
3. انسخ مفتاح API لاستخدامه في LocalCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في LocalCode، يجب سحب معلومات النموذج محليا:
...
LocalCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق LocalCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>LocalCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **LocalCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ LocalCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات localcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات localcode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Localcode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات localcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع localcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `localcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ LocalCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `localcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات localcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات localcode.

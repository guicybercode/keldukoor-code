...
مع LocalCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم LocalCode سمتنا `localcode`.
...
يأتي LocalCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق LocalCode مظهر terminal لديهم
...
يدعم LocalCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/localcode/themes/*.json` أو `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.localcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.localcode/themes/*.json`

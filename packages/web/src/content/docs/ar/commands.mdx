...
أنشئ الملف `.localcode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات LocalCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ LocalCode:
...
- على مستوى النظام: `~/.config/localcode/commands/`
- لكل مشروع: `.localcode/commands/`
...
يتضمن localcode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

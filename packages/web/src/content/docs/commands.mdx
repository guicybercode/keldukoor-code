...
Create `.localcode/commands/test.md`:
...
You can add custom commands through the LocalCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your LocalCode [config](/docs/config):
...
- Per-project: `.localcode/commands/`
...
localcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć localcode.
---

Wtyczki umożliwiające wyłączenie localcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania localcode.
...
- `.localcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/localcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/localcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/localcode/localcode.json`)
2. Konfiguracja projektu (`localcode.json`)
3. Globalny katalog wtyczek (`~/.config/localcode/plugins/`)
4. Katalog wtyczek projektu (`.localcode/plugins/`)
...
localcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z localcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do localcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej localcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj localcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do localcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod localcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://localcode.dev/docs/sdk).

...
Umiejętności agenta pozwalają localcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
localcode przeszukuje te lokalizacje:

- Project config: `.localcode/skills/<name>/SKILL.md`
- Global config: `~/.config/localcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu localcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.localcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.localcode/skills/git-release/SKILL.md` w ten sposób:
...
localcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `localcode.json`:
...
**Dla agentów wbudowanych** (w `localcode.json`):

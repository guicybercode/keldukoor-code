...
W LocalCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
LocalCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
LocalCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

LocalCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `localcode.json`:
...
- Globalnie: `~/.config/localcode/agents/`
- W projekcie: `.localcode/agents/`
...
Jeśli nie zostanie skonfigurowana, LocalCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji LocalCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji LocalCode w `provider/model-id`. Na przykład, [LocalCode Zen](/docs/zen), użyj `localcode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `localcode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/localcode).
:::

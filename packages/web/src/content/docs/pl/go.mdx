...
LocalCode Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
LocalCode Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w LocalCode. Subskrybujesz LocalCode Go i
...
LocalCode Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

LocalCode Go działa jak każdy inny dostawca w LocalCode.

1. Logujesz się do **<a href={console}>LocalCode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `LocalCode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować LocalCode Go.
:::
...
LocalCode Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji LocalCode
używa formatu `localcode-go/<model-id>`. Na przykład dla Kimi K2.5 należy użyć
`localcode-go/kimi-k2.5` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z LocalCode.

...
Możesz dodać dodatkowe informacje do localcode za pomocą _Model Context Protocol_, w skrócie MCP. localcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [localcode Config](https://localcode.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/localcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
localcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, localcode:
...
Jeśli serwer wymaga uwierzytelnienia, localcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `localcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji localcode bezpieczne przechowa tokeny w `~/.local/share/localcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w localcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie localcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć localcode z kontem Sentry.

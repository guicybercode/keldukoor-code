---
title: Serwer
description: Interakcja z serwerem localcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `localcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient localcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `localcode.local` |
...
Ustaw `LOCALCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `localcode` lub ustaw `LOCALCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `localcode serve`, jak i `localcode web`.
...
Kiedy uruchomisz `localcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera localcode do programowej interakcji z localcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez localcode i programową interakcję z kodem otwartym.

Możesz uruchomić `localcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
localcode TUI działa, `localcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki localcode [IDE](/docs/ide).
...
Serwer localcode udostępnia następujące interfejsy API.

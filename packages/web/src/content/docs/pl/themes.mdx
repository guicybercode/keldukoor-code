...
Dzięki localcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie localcode używa naszego własnego motywu `localcode`.
...
localcode ma kilka wbudowanych motywów.
...
- Chcą, aby LocalCode pasował do wyglądu ich terminala
...
localcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/localcode/themes/*.json` lub `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.localcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.localcode/themes/*.json`

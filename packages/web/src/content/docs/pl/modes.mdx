...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji localcode.
...
te lub skonfiguruj własne za pomocą konfiguracji localcode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.localcode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `localcode.json`:
...
- Globalnie: `~/.config/localcode/modes/`
...
Jeśli nie ma wpływu na temperaturę, localcode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna localcode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.localcode/modes/` dla trybów zapisanych dla projektu lub `~/.config/localcode/modes/` dla trybów globalnych:

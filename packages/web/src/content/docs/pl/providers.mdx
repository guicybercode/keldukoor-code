---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w localcode.
---

import config from "../../../../config.mjs"
export const console = config.console

localcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji localcode.
...
w `~/.local/share/localcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim localcode
config.
...
LocalCode Zen to lista modeli dostarczonych przez zespół localcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z localcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od LocalCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `LocalCode Zen` i przejdź do [localcode.dev/auth](https://localcode.dev/zen).
...
Działa jak każdy inny dostawca w localcode i jest całkowicie opcjonalny w użyciu.

---

## LocalCode Go

LocalCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół localcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z localcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `LocalCode Go` i przejdź do [localcode.dev/auth](https://localcode.dev/zen).
...
Aby używać Amazon Bedrock z localcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania localcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `localcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w localcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod localcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji localcode.
...
localcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować localcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`localcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth).

##### Konfiguracja

Customize through `localcode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem localcode:
...
Aby używać Google Vertex AI z localcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania localcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą localcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session), aby automatycznie rejestrować każdą konwersację localcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja localcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować localcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować localcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować localcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem localcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/localcode).
:::
...
Aby korzystać z Ollama Cloud z localcode:
...
3. Skopiuj klucz API do użycia w localcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w localcode musisz pobrać informacje o modelu lokalnie:
...
LocalCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół localcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>LocalCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API localcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji localcode.
...
5. Możesz je także dostosować za pomocą konfiguracji localcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z localcode:
...
Możesz także dostosować modele za pomocą konfiguracji localcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z localcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `localcode.json` w katalogu projektu:
...
Pola `limit` pozwalają localcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `localcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację localcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji localcode.

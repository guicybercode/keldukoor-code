...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. LocalCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w LocalCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `LOCALCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy LocalCode lub gdy zmienna środowiskowa `LOCALCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu LocalCode:

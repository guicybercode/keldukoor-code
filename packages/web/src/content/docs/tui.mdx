---
title: TUI
description: Using the LocalCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running LocalCode starts the TUI for the current directory.
...
When using the LocalCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to LocalCode. Allows you to select from available providers and add their API keys.
...
Exit LocalCode. _Aliases_: `/quit`, `/q`
...
This is separate from `localcode.json`, which configures server/runtime behavior.
...
Use `LOCALCODE_TUI_CONFIG` to load a custom TUI config path.

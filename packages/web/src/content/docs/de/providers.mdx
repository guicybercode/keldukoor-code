---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer LocalCode-Konfiguration.
...
in `~/.local/share/localcode/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem LocalCode anpassen
config.
...
LocalCode Zen ist eine Liste von Modellen, die vom LocalCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit LocalCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit LocalCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie LocalCode aus und gehen Sie zu [localcode.dev/auth](https://localcode.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in LocalCode und ist völlig optional.

---

## LocalCode Go

LocalCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom LocalCode-Team getestet und verifiziert wurden, dass sie gut mit LocalCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `LocalCode Go` und gehen Sie zu [localcode.dev/auth](https://localcode.dev/zen).
...
So verwenden Sie Amazon Bedrock mit LocalCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie LocalCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `localcode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in LocalCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit LocalCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre LocalCode-Konfiguration hinzufügen.
...
um das Token im LocalCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
LocalCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von LocalCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`localcode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `localcode.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit LocalCode:
...
So verwenden Sie Google Vertex AI mit LocalCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie LocalCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über LocalCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session)-Plugin, um jede LocalCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede LocalCode-Konversation als separate Sitzung aufgeführt.
...
Sie können LocalCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können LocalCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können LocalCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für LocalCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/localcode).
:::
...
So verwenden Sie Ollama Cloud mit LocalCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in LocalCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in LocalCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
LocalCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom LocalCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>LocalCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren LocalCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre LocalCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre LocalCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit LocalCode:
...
Sie können Modelle auch über Ihre LocalCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit LocalCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `localcode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann LocalCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `localcode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die LocalCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer LocalCode-Konfiguration übereinstimmt.

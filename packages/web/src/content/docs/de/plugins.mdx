---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um LocalCode zu erweitern.
---

Mit Plugins können Sie LocalCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von LocalCode zu ändern.
...
- `.localcode/plugins/` – Plugins auf Projektebene
- `~/.config/localcode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/localcode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/localcode/localcode.json`)
2. Projektkonfiguration (`localcode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/localcode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.localcode/plugins/`)
...
LocalCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein LocalCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie LocalCode erweitern können.
...
:::note
Wenn Sie die LocalCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass LocalCode `.env`-Dateien liest:
...
Plugins können LocalCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das LocalCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für LocalCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://localcode.dev/docs/sdk).

---
title: Server
description: Nutze den localcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `localcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein localcode-Client nutzen kann.
...
Setze `LOCALCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `localcode`, kann aber mit `LOCALCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `localcode serve` und `localcode web`.
...
Wenn du `localcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den localcode-Server, um localcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `localcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `localcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die LocalCode-[IDE](/docs/ide)-Plugins.
...
Der localcode-Server stellt folgende APIs bereit.

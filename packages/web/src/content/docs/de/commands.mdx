...
Erstellen Sie `.localcode/commands/test.md`:

```md title=".localcode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die LocalCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem LocalCode [config](/docs/config):

```json title="localcode.jsonc" {4-12}
{
...
- Global: `~/.config/localcode/commands/`
- Pro Projekt: `.localcode/commands/`

```markdown title="~/.config/localcode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".localcode/commands/component.md"
---
...
```md title=".localcode/commands/create-file.md"
---
...
```md title=".localcode/commands/analyze-coverage.md"
---
...
```md title=".localcode/commands/review-changes.md"
---
...
```md title=".localcode/commands/review-component.md"
---
...
LocalCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).

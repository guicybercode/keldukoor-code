...
LocalCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
LocalCode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in LocalCode. Du abonnierst LocalCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
LocalCode zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit LocalCode funktionieren.
:::
...
LocalCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

LocalCode Go funktioniert wie jeder andere Provider in LocalCode.

1. Du meldest dich bei **<a href={console}>LocalCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `LocalCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann LocalCode Go abonnieren.
:::
...
LocalCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner LocalCode Config verwendet das Format `localcode-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `localcode-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben LocalCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit LocalCode zu nutzen.

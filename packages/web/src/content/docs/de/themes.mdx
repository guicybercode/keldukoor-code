...
Mit LocalCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet LocalCode das `localcode`-Theme.
...
LocalCode liefert mehrere eingebaute Themes mit.
...
- LocalCode optisch ans Terminal anpassen wollen
...
LocalCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/localcode/themes/*.json` oder `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.localcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.localcode/themes/*.json`

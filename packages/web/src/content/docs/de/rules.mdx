---
title: Regeln
description: Lege eigene Anweisungen fuer localcode fest.
---

Du kannst localcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in localcode den Befehl `/init` aus.
...
So kann localcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht localcode sie zu erweitern.
...
localcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/localcode/AGENTS.md` speichern.
Diese gelten in allen localcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt LocalCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/localcode/AGENTS.md` existiert)
...
Beim Start sucht localcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/localcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `localcode.json` oder in `~/.config/localcode/localcode.json` angeben.
...
localcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit localcode.json

Empfohlen ist das `instructions`-Feld in der `localcode.json`:
...
Du kannst localcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `localcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

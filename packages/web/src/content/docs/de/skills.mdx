...
Agent Skills erlauben LocalCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
LocalCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft LocalCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.localcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.localcode/skills/git-release/SKILL.md` zum Beispiel so:
...
LocalCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `localcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `localcode.json`):

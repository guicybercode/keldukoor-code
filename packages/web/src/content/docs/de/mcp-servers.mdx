...
Sie können externe Tools zu LocalCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. LocalCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [LocalCode Config](https://localcode.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/localcode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
LocalCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird LocalCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von LocalCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `localcode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert LocalCode die Token sicher in `~/.local/share/localcode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in LocalCode verfügbar. Sie können sie also wie jedes andere Tool über die LocalCode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und LocalCode mit Ihrem Sentry-Konto zu verbinden.

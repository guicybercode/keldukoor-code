---
title: Providers
description: Using any LLM provider in LocalCode.
---

import config from "../../../config.mjs"
export const console = config.console

LocalCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your LocalCode config.
...
You can customize the providers through the `provider` section in your LocalCode
config.
...
LocalCode Zen is a list of models provided by the LocalCode team that have been
tested and verified to work well with LocalCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with LocalCode Zen.
:::

1. Run the `/connect` command in the TUI, select `LocalCode Zen`, and head to [localcode.dev/auth](https://localcode.dev/zen).
...
It works like any other provider in LocalCode and is completely optional to use.

---

## LocalCode Go

LocalCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the LocalCode team that have been
tested and verified to work well with LocalCode.

1. Run the `/connect` command in the TUI, select `LocalCode Go`, and head to [localcode.dev/auth](https://localcode.dev/zen).
...
To use Amazon Bedrock with LocalCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running localcode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `localcode.json`:
...
LocalCode. Anthropic explicitly prohibits this.

Previous versions of LocalCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in LocalCode with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for localcode to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your localcode config.
...
GitLab Duo support in LocalCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

LocalCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
LocalCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock LocalCode
to only use your own GitLab-hosted instance, add the following to your
`localcode.json` file. It is also recommended to disable session sharing.
...
More documentation on [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth) homepage.
...
When a `duo-workflow-*` model is selected, LocalCode will:
...
4. Route tool execution requests through LocalCode's permission-gated tool system
...
To use your GitHub Copilot subscription with localcode:
...
To use Google Vertex AI with LocalCode:
...
2. Set the required environment variables:
   ...
   Set them while running localcode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through localcode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) plugin to automatically log each LocalCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each LocalCode conversation listed as a separate session.
...
You can configure localcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure localcode to use local models through LM Studio.
...
You can configure localcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for LocalCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/localcode) for details.
:::
...
To use Ollama Cloud with LocalCode:
...
3. Copy the API key for use in LocalCode.
...
6. **Important**: Before using cloud models in LocalCode, you must pull the model information locally:
...
LocalCode Zen is a list of tested and verified models provided by the LocalCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>LocalCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Enter your LocalCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your localcode config.
...
5. You can also customize them through your localcode config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Localcode:
...
You can also customize models through your localcode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with localcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `localcode.json` file in your project directory:
...
The `limit` fields allow LocalCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `localcode auth list` to see if the credentials
...
2. For custom providers, check the localcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your localcode config.

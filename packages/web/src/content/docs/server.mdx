---
title: Server
description: Interact with localcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `localcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an localcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `localcode.local` |
...
Set `LOCALCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `localcode`, or set `LOCALCODE_SERVER_USERNAME` to override it. This applies to both `localcode serve` and `localcode web`.
...
When you run `localcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the localcode server to interact with localcode programmatically.
:::

This architecture lets localcode support multiple clients and allows you to interact with localcode programmatically.

You can run `localcode serve` to start a standalone server. If you have the
localcode TUI running, `localcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the LocalCode [IDE](/docs/ide) plugins.
...
The localcode server exposes the following APIs.

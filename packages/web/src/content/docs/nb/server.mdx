---
title: Tjener
description: Samhandle med LocalCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `localcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en LocalCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `localcode.local` |
...
Sett `LOCALCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `localcode` som standard, eller sett `LOCALCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `localcode serve` og `localcode web`.
...
Når du kjører `localcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk LocalCode-serveren til å samhandle med LocalCode programmatisk.
:::

Denne arkitekturen lar LocalCode støtte flere klienter og lar deg samhandle med LocalCode programmatisk.

Du kan kjøre `localcode serve` for å starte en frittstående server. Hvis du har
LocalCode TUI kjører, vil `localcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av LocalCode [IDE](/docs/ide) plugins.
...
localcode-serveren viser følgende APIer.

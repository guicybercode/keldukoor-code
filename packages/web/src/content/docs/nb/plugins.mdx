...
description: Skriv dine egne programtillegg for å utvide LocalCode.
---

Plugins lar deg utvide LocalCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til LocalCode.
...
- `.localcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/localcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/localcode/localcode.json`)
2. Prosjektkonfigurasjon (`localcode.json`)
3. Global plugin-katalog (`~/.config/localcode/plugins/`)
4. Prosjektpluginkatalog (`.localcode/plugins/`)
...
LocalCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En LocalCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide LocalCode.
...
:::note
Hvis du bruker LocalCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre LocalCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til LocalCode:
...
`tool`-hjelperen lager et tilpasset verktøy som LocalCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for LocalCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://localcode.dev/docs/sdk) for detaljer.

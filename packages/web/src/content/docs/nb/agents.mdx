...
Det er to typer agenter i LocalCode; primære agenter og underagenter.
...
LocalCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
LocalCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

LocalCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `localcode.json`:
...
- Globalt: `~/.config/localcode/agents/`
- Per prosjekt: `.localcode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker LocalCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale LocalCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i LocalCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [LocalCode Zen](/docs/zen), vil du bruke `localcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `localcode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/localcode).
:::

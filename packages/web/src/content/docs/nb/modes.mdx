...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i LocalCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i LocalCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom LocalCode-konfigurasjonen.
...
LocalCode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.localcode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `localcode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/localcode/modes/`
- Prosjekt: `.localcode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker LocalCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale LocalCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.localcode/modes/` for prosjektspesifikke moduser eller `~/.config/localcode/modes/` for globale moduser:

---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i LocalCode-konfigurasjonen.
...
i `~/.local/share/localcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i LocalCode
konfig.
...
LocalCode Zen er en liste over modeller levert av LocalCode-teamet som har vært
testet og verifisert for å fungere godt med LocalCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med LocalCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `LocalCode Zen` og gå til [localcode.dev/auth](https://localcode.dev/zen).
...
Det fungerer som alle andre leverandører i LocalCode og er helt valgfritt å bruke.

---

## LocalCode Go

LocalCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av LocalCode-teamet som har vært
testet og verifisert for å fungere godt med LocalCode.

1. Kjør kommandoen `/connect` i TUI, velg `LocalCode Go`, og gå til [localcode.dev/auth](https://localcode.dev/zen).
...
Slik bruker du Amazon Bedrock med LocalCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører LocalCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `localcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i localcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at LocalCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom LocalCode-konfigurasjonen.
...
for å lagre token i LocalCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
LocalCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse LocalCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`localcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `localcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med LocalCode:
...
Slik bruker du Google Vertex AI med LocalCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører LocalCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom LocalCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) plugin for automatisk å logge hver LocalCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver LocalCode-samtale oppført som en separat økt.
...
Du kan konfigurere LocalCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere LocalCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere LocalCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for LocalCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/localcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med LocalCode:
...
3. Kopier API-nøkkelen for bruk i LocalCode.
...
6. **Viktig**: Før du bruker skymodeller i LocalCode, må du hente modellinformasjonen lokalt:
...
LocalCode Zen er en liste over testede og verifiserte modeller levert av LocalCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>LocalCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn LocalCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom LocalCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom LocalCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med LocalCode:
...
Du kan også tilpasse modeller gjennom LocalCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med LocalCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `localcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar LocalCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `localcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk LocalCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i LocalCode-konfigurasjonen.

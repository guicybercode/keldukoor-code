...
Du kan legge til eksterne verktøy i LocalCode ved å bruke _Model Context Protocol_ eller MCP. LocalCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [LocalCode Config](https://localcode.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="localcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/localcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="localcode.jsonc" {15}
{
  "$schema": "https://localcode.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
LocalCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil LocalCode:
...
Hvis serveren krever autentisering, vil LocalCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `localcode mcp auth <server-name>`.
...
```json title="localcode.json" {7-11}
{
  "$schema": "https://localcode.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil LocalCode lagre tokens sikkert i `~/.local/share/localcode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i LocalCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom LocalCode-konfigurasjonen som alle andre verktøy.
...
```json title="localcode.json" {11, 14-18}
{
  "$schema": "https://localcode.dev/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble LocalCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver LocalCode-samtale oppført som en separat økt.

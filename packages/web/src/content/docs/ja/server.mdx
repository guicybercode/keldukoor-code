---
title: サーバー
description: HTTP 経由で LocalCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`localcode serve` コマンドは、LocalCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `localcode.local` |
...
HTTP 基本認証でサーバーを保護するには、`LOCALCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `localcode` になるか、`LOCALCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`localcode serve` と `localcode web` の両方に当てはまります。
...
`localcode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
LocalCode サーバーを使用して、プログラムで LocalCode と対話します。
:::

`localcode serve` を実行してスタンドアロンサーバーを起動できます。すでに LocalCode TUI を実行している場合でも、`localcode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、LocalCode [IDE](/docs/ide) プラグインによって使用されます。

---
title: プラグイン
description: LocalCode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで LocalCode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、LocalCode のデフォルトの動作を変更したりできます。
...
- `.localcode/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/localcode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/localcode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/localcode/localcode.json`)
2. プロジェクト設定 (`localcode.json`)
3. グローバルプラグインディレクトリ (`~/.config/localcode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.localcode/plugins/`)
...
LocalCode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための LocalCode SDK クライアント。
...
ここでは、LocalCode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://localcode.dev/docs/sdk) を参照してください。
...
LocalCode が `.env` ファイルを読み取らないようにします。
...
プラグインは LocalCode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、LocalCode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して LocalCode で使用できます。

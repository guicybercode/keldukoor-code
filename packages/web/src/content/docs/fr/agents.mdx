...
Il existe deux types d'agents dans LocalCode ; agents primaires et sous-agents.
...
LocalCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
LocalCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

LocalCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `localcode.json` :
...
- Global : `~/.config/localcode/agents/`
- Par projet : `.localcode/agents/`

```markdown title="~/.config/localcode/agents/review.md"
---
...
Si aucune température n'est spécifiée, LocalCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale LocalCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration LocalCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [LocalCode Zen](/docs/zen), vous utiliserez `localcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="localcode.json" {3-6,9-12}
{
  "$schema": "https://localcode.dev/config.json",
  "tools": {
    "write": true,
...
```json title="localcode.json" {3-5,8-10}
{
  "$schema": "https://localcode.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `localcode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/localcode).
:::
...
```markdown title="~/.config/localcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/localcode/agents/security-auditor.md"
---

---
title: TUI
description: Utilisation de l’interface utilisateur du terminal LocalCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de LocalCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le LocalCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à LocalCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez LocalCode. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `localcode.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `LOCALCODE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.

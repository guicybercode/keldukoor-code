---
title: Serveur
description: Interagissez avec le serveur localcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `localcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client localcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `localcode.local` |
...
Définissez `LOCALCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `localcode`, ou définissez `LOCALCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `localcode serve` et à `localcode web`.
...
Lorsque vous exécutez `localcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur localcode pour interagir avec localcode par programmation.
:::

Cette architecture permet à localcode de prendre en charge plusieurs clients et vous permet d'interagir avec localcode par programme.

Vous pouvez exécuter `localcode serve` pour démarrer un serveur autonome. Si vous avez le
localcode TUI en cours d'exécution, `localcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins LocalCode [IDE](/docs/ide).
...
Le serveur localcode expose les API suivantes.

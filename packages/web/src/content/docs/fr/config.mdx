---
title: Configuration
description: Utilisation de la configuration JSON LocalCode.
---

Vous pouvez configurer LocalCode à l'aide d'un fichier de configuration JSON.

---

## Format

LocalCode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="localcode.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/localcode`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/localcode/localcode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`LOCALCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`localcode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.localcode`** - agents, commandes, plugins
6. **Configuration en ligne** (`LOCALCODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.localcode` et `~/.config/localcode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/localcode`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/localcode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale LocalCode dans `~/.config/localcode/localcode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/localcode/tui.json`.
...
Ajoutez `localcode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque LocalCode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `LOCALCODE_CONFIG`.

```bash
export LOCALCODE_CONFIG=/path/to/my/custom-config.json
localcode run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `LOCALCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.localcode`, et devrait suivre la même structure.

```bash
export LOCALCODE_CONFIG_DIR=/path/to/my/config-directory
localcode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.localcode`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`localcode.dev/config.json`**](https://localcode.dev/config.json).

La configuration TUI utilise [**`localcode.dev/tui.json`**](https://localcode.dev/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://localcode.dev/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Utilisez `LOCALCODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `localcode.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `localcode serve` et `localcode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur LocalCode.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `localcode.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration LocalCode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, LocalCode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/localcode/agents/` ou `.localcode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, LocalCode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`localcode run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/localcode/commands/` ou `.localcode/commands/`. [En savoir plus ici](/docs/commands).
...
LocalCode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, localcode **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent LocalCode avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.localcode/plugins/` ou `~/.config/localcode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="localcode.json"
{
  "$schema": "https://localcode.dev/config.json",
  "plugin": ["localcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre LocalCode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.

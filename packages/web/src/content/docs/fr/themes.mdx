...
Avec LocalCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, LocalCode utilise notre propre thème `localcode`.
...
LocalCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que LocalCode corresponde à l'apparence de leur terminal
...
LocalCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/localcode/themes/*.json` ou `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.localcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.localcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/localcode/themes
vim ~/.config/localcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .localcode/themes
vim .localcode/themes/my-theme.json
```
...
{
  "$schema": "https://localcode.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

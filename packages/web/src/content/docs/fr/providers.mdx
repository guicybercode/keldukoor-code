---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en LocalCode.
---

import config from "../../../../config.mjs"
export const console = config.console

LocalCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration LocalCode.
...
en `~/.local/share/localcode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre LocalCode
configuration.
...
LocalCode Zen est une liste de modèles fournis par l'équipe LocalCode qui ont été
testé et vérifié pour fonctionner correctement avec LocalCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par LocalCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez localcode et dirigez-vous vers [localcode.dev/auth](https://localcode.dev/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans LocalCode et son utilisation est totalement facultative.

---

## LocalCode Go

LocalCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe LocalCode qui ont été
testé et vérifié pour fonctionner correctement avec LocalCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `LocalCode Go` et rendez-vous sur [localcode.dev/auth](https://localcode.dev/zen).
...
Pour utiliser Amazon Bedrock avec LocalCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de localcode :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY localcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile localcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX localcode
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `localcode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans LocalCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que localcode fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX localcode
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX localcode
   ```
...
Vous pouvez également ajouter des modèles via votre configuration localcode.
...
pour stocker le jeton dans le stockage d'authentification localcode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
LocalCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller LocalCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `localcode.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth).

##### Configuration

Personnalisez via `localcode.json` :
...
{
  "$schema": "https://localcode.dev/config.json",
  "plugin": ["localcode-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec localcode :
...
Pour utiliser Google Vertex AI avec LocalCode :
...
Définissez-les lors de l'exécution de localcode.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id localcode
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via localcode, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/localcode/localcode.jsonc"
{
  "$schema": "https://localcode.dev/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) pour enregistrer automatiquement chaque conversation LocalCode en tant que session dans Helicone.

```bash
npm install -g localcode-helicone-session
```
...
```json title="localcode.json"
{
  "plugin": ["localcode-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation LocalCode répertoriée comme une session distincte.
...
Vous pouvez configurer localcode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="localcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://localcode.dev/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer localcode pour utiliser des modèles locaux via LM Studio.

```json title="localcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://localcode.dev/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer localcode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour LocalCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/localcode) pour plus de détails.
:::

```json title="localcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://localcode.dev/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec LocalCode :
...
3. Copiez la clé API à utiliser dans LocalCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans LocalCode, vous devez extraire les informations du modèle localement :
...
LocalCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe LocalCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>LocalCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **LocalCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé LocalCode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration localcode.
...
5. Vous pouvez également les personnaliser via votre configuration localcode. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' localcode
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group localcode
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Localcode :
...
Vous pouvez également personnaliser les modèles via votre configuration localcode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec localcode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in localcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `localcode.json` dans le répertoire de votre projet :

   ```json title="localcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://localcode.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="localcode.json" {9,11,17-20}
{
  "$schema": "https://localcode.dev/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à LocalCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `localcode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration localcode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration localcode.

...
LocalCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
LocalCode Go şu anda beta olarak mevcut.
:::

Go, LocalCode'daki diğer sağlayıcılar gibi çalışır. LocalCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve LocalCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
LocalCode ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
LocalCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

LocalCode Go, LocalCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>LocalCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `LocalCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye LocalCode Go'ya abone olabilir.
:::
...
LocalCode Go aşağıdaki limitleri içerir:
...
LocalCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `localcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `localcode-go/kimi-k2.5` kullanmalısınız.
...
LocalCode Go'yu şu amaçlarla oluşturduk:
...
4. LocalCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

...
`.localcode/commands/test.md` oluştur:
...
localcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

localcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/localcode/commands/`
- Proje başına: `.localcode/commands/`
...
localcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).

...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. localcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
localcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `LOCALCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `LOCALCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca localcode provider kullanılırken veya `LOCALCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

localcode başlatırken etkinleştirmek için:

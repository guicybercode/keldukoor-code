...
Ajan becerileri, localcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
localcode su konumlari tarar:

- Proje konfigurasyonu: `.localcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/localcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda localcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.localcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/localcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.localcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
localcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`localcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`localcode.json` icinde):

---
title: Eklentiler
description: LocalCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek LocalCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya LocalCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.localcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/localcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/localcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/localcode/localcode.json`)
2. Project config (`localcode.json`)
3. Global eklenti dizini (`~/.config/localcode/plugins/`)
4. Proje eklenti dizini (`.localcode/plugins/`)
...
LocalCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir LocalCode SDK istemcisi.
...
LocalCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
LocalCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
LocalCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca LocalCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, LocalCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra LocalCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://localcode.dev/docs/sdk) bakın.

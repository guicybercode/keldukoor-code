...
_Model Bağlam Protokolü_ veya MCP kullanarak localcode'a harici araçlar ekleyebilirsiniz. localcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [localcode Config](https://localcode.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/localcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
localcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde localcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa localcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `localcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından localcode, belirteçleri `~/.local/share/localcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra localcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi localcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve localcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

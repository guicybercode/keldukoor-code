---
title: Zen
description: localcode tarafından sağlanan seçilmiş modeller listesi.
---
...
LocalCode Zen, localcode ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
LocalCode Zen şu anda beta aşamasındadır.
:::

Zen, localcode'daki diğer sağlayıcılar gibi çalışır. LocalCode Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve localcode kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
localcode ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
LocalCode Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

LocalCode Zen, localcode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>LocalCode Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, LocalCode Zen'i seçin ve API anahtarınızı yapıştırın.
...
localcode yapılandırmanızdaki [model kimliği](/docs/config/#models) `localcode/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `localcode/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için LocalCode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için LocalCode'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
LocalCode Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman LocalCode ile başka bir sağlayıcıyı kullanmanıza izin vermek.

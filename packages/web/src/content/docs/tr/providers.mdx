---
title: Sağlayıcılar
description: localcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

localcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı localcode yapılandırmanızda yapılandırın.
...
`~/.local/share/localcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları localcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
LocalCode Zen, localcode ekibi tarafından sağlanan modellerin bir listesidir.
localcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz LocalCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, localcode'u seçin ve [localcode.dev/auth](https://localcode.dev/auth)'ye gidin.
...
localcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## LocalCode Go

LocalCode Go, localcode ile iyi çalıştığı test edilmiş ve doğrulanmış, localcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `LocalCode Go`'yu seçin ve [localcode.dev/auth](https://localcode.dev/zen) adresine gidin.
...
Amazon Bedrock'u localcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   localcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `localcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi localcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   localcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Localcode yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in localcode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
localcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. localcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`localcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [localcode-gitlab-auth](https://www.npmjs.com/package/localcode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`localcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi localcode'la kullanmak için:
...
Google Vertex AI'yi localcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları localcode çalıştırılırken ayarlayın.
...
Helicone'un localcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her localcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [localcode-helicone-session](https://github.com/H2Shami/localcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her localcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
localcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla localcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla localcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini localcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/localcode)'a bakın.
:::
...
Ollama Cloud'u localcode ile kullanmak için:
...
3. localcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: localcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
LocalCode Zen, localcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>LocalCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **LocalCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. localcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   localcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca localcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Localcode ile kullanmak için:
...
Modelleri localcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı localcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `localcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları localcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `localcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için localcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, localcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.

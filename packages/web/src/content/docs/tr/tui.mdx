---
title: TUI
description: LocalCode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

LocalCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

LocalCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
LocalCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
LocalCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
LocalCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `localcode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `LOCALCODE_TUI_CONFIG` kullanın.

...
localcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak localcode kendi `localcode` temasını kullanır.
...
localcode birden fazla yerleşik temayla gelir.
...
- localcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
localcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/localcode/themes/*.json` veya `$XDG_CONFIG_HOME/localcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.localcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.localcode/themes/*.json`

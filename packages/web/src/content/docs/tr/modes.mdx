...
:::caution
Modlar artık localcode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

localcode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya localcode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
localcode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.localcode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`localcode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, localcode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global localcode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.localcode/modes/` veya genel modlar için `~/.config/localcode/modes/`'de mod dosyaları oluşturun:

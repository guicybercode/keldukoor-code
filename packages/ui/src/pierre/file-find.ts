...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("localcode-find")
  api.delete("localcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("localcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("localcode-find", new Highlight(...rest))
    return true
  }

import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@localcode/sdk/v2"
import { SessionTurn } from "@localcode/ui/session-turn"
import { SessionReview } from "@localcode/ui/session-review"
import { DataProvider } from "@localcode/ui/context"
import { FileComponentProvider } from "@localcode/ui/context/file"
import { WorkerPoolProvider } from "@localcode/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@localcode/ui/logo"
import { IconButton } from "@localcode/ui/icon-button"
import { ProviderIcon } from "@localcode/ui/provider-icon"
import { iife } from "@localcode/util/iife"
import { Binary } from "@localcode/util/binary"
import { NamedError } from "@localcode/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@localcode/ui/tabs"
import { MessageNav } from "@localcode/ui/message-nav"
import { FileSSR } from "@localcode/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@localcode/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/localcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | LocalCode</Title>
              </Show>
              <Meta name="description" content="localcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/localcode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://localcode.dev/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
